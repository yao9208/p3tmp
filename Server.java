		...
		if(ismaster){
			...
			Runnable schedule= new Runnable(){
				public void run(){
					...
					int shredshould=1;
					long lasttime=0, lastmidtime=0, lastjudge=0, time=0;
					int prevLength=0;
					long sumlen=0;
					double sum=0.0;
					boolean flag=false;
					if(hour==20) shredshould--;
					while(true){
						long curTime;					
						...
						boolean first=true;
						if(queueLength>=middlenum && middlenum<10 &&(curTime-lastmidtime)>5500 &&!flag){
							miduptimes++;
							sum += queueLength;
							if(miduptimes>=5){
						...
						if(frontqueueLength>=(frontnum+1)*0.5 && frontnum<2 && 
								(curTime-lasttime)>8000 &&(curTime-startTime)>3000 && !flag){
							frontuptimes++;
						...
						if(time>=4 && (curTime-lastjudge)>500 && ms.middleList.size()>2 && (curTime-startTime)>30000){
							if(sumlen/5>1500){
								...
								if(hour==20)flag=true;
								try {
									shutdownServer(serverid, ip, port);
								} catch (Exception e) {
									e.printStackTrace();
								}
								if(ms.frontlist.size()>shredshould){
